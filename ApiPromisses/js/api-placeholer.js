class MinhaApi extends HTMLElement {
    ...
    connectedCallback(){
        ...
        formulario.addEventListener("submit",(event) => this.enviarDados(event));
    }

    enviarDados(event){
        event.preventDefault();

        const nome = this.shadowRoot.querySelector("#nome").value.trim();

        const email = this.shadowRoot.querySelector("#email").value.trim();

        const resultado = this.shadowRoot.querySelector("#resultado");

        resultado.innerHTML = "<p>Enviando dados...</p>";
